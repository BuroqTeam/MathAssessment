...
public class Pattern_23 : GeneralTest
{
    ...
    private void OnEnable()
    {
        ...
        DisplayQuestion(Pattern_23Obj.title);
    }
    ...
    void ReadFromJson()
    {
        ...
        JObject jo = Mbt.LoadJsonPath(jsonObj, "Pattern_23");
        Pattern_23Obj = jo.ToObject<Data_23>();
        CreatePrefabs();
    }
    ...
    void CreatePrefabs()
    {
        int optionsCount = Pattern_23Obj.options.Count;
        /*Pattern_9Obj.options = */
        Pattern_23Obj.options.ShuffleList();
    ...
    void WriteToPrefab()
    {
        ...
        {
            ComparisonObjects[i].transform.GetChild(0).transform.GetChild(0).GetComponent<TEXDraw>().text = Pattern_23Obj.options[i].left;
            ComparisonObjects[i].transform.GetChild(1).GetComponent<DropDownP_23>().CorrectAnswer = Pattern_23Obj.options[i].sign.ToString();
            ComparisonObjects[i].transform.GetChild(1).GetComponent<DropDownP_23>().Pattern_23 = this;
            ComparisonObjects[i].transform.GetChild(2).transform.GetChild(0).GetComponent<TEXDraw>().text = Pattern_23Obj.options[i].right;
        }
    }
    ...
    public void CheckAllAnswers()
    {
        ...
        int n = Pattern_23Obj.options.Count;
        totalFullAns = 0;
        totalCorrectAns = 0;

        for (int i = 0; i < n; i++)
        {
            string currentAnswer = ComparisonObjects[i].transform.GetChild(1).GetComponent<DropDownP_23>().CurrentAnswer;
            string correctAnswer = ComparisonObjects[i].transform.GetChild(1).GetComponent<DropDownP_23>().CorrectAnswer;
        ...
        correctCount = 0;
        for (int i = 0; i < Pattern_23Obj.options.Count; i++)
        {
            string currentAnswer = ComparisonObjects[i].transform.GetChild(1).GetComponent<DropDownP_23>().CurrentAnswer;
    ...
    void PatternButtonBlue()        // Pattern scriptidagi isEdited ni true yoki false qilib beruvchi metod.
    {
        ...
        for (int i = 0; i < ComparisonObjects.Count; i++)
        {
            int currentAnswerLength = ComparisonObjects[i].transform.GetChild(1).GetComponent<DropDownP_23>().CurrentAnswer.Length;
            string currentAnswerStr = ComparisonObjects[i].transform.GetChild(1).GetComponent<DropDownP_23>().CurrentAnswer;
            string initialStr = ComparisonObjects[i].transform.GetChild(1).GetComponent<DropDownP_23>().InitialStr;
...
{
    public string title;
    public List<Options_23> options = new();
}

[SerializeField]
public class Options_23
{
    public string left;
    public char sign;
    public string right;
