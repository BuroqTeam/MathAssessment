...
public class Pattern_17 : GeneralTest
{
    ...
    public void ReadFromJson()
    {
        ...
        JObject jo = Mbt.LoadJsonPath(jsonObj, "Pattern_17");
        Pattern_1Obj = jo.ToObject<Data_17>();
        CreatePrefabs2();
    }
    ...
    public void CreatePrefabs2()
    {
        ...
        for (int i = 0; i < ABCD.Count; i++)
        {
            ...
            ABCD[i].GetComponent<AnswerPattern17>().PatternOne = this;

            if (likeName.Contains('*'))
            {
                ABCD[i].GetComponent<AnswerPattern17>()._IsTrue = true;
                likeName = likeName.Replace("[*]", "");
            }
            ABCD[i].GetComponent<AnswerPattern17>().WriteCurrentAnswer(likeName);
        }
    ...
    public void UnClickedButtons()
    {
        for (int i = 0; i < ABCD.Count; i++)
        {
            ABCD[i].GetComponent<AnswerPattern17>().DisableObject();
        }
